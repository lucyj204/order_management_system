...
def add_product_order(args):
    ...
    db_utils.add_order_line(order_id=order_id, product_name=product_name, product_quantity=product_quantity)
...
def show_single_order(order_id):
    ...
    output_lines = [total_order_return] + [f'{ol.product_name} {ol.product_quantity} {ol.status}' for ol in order_lines ]
    return '\n'.join(output_lines)
